...
def print_stats(db_url):
    ...
    with engine.connect() as conn:
        ...
        for r in conn.execute(
            text(
                "SELECT plan_type, COUNT(*) n, ROUND(COUNT(*)*100.0/SUM(COUNT(*)) OVER(),1) pct FROM raw.members GROUP BY 1 ORDER BY 2 DESC"
            )
        ).fetchall():
        ...
        row = conn.execute(
            text(
                "SELECT COUNT(*), SUM(CASE WHEN end_date IS NOT NULL THEN 1 ELSE 0 END), ROUND(SUM(CASE WHEN end_date IS NOT NULL THEN 1 ELSE 0 END)*100.0/COUNT(*),1) FROM raw.subscriptions"
            )
        ).fetchone()
        print(
            f"  Total: {row[0]:,}  Churned: {row[1]:,} ({row[2]}%)  Active: {row[0]-row[1]:,}"
        )

        print(f"\n--- Top 10 Feature Events ---")
        for r in conn.execute(
            text(
                "SELECT event_name, COUNT(*) n FROM raw.feature_events GROUP BY 1 ORDER BY 2 DESC LIMIT 10"
            )
        ).fetchall():
        ...
        print(f"\n--- Experiments ---")
        for r in conn.execute(
            text("SELECT * FROM raw.experiments ORDER BY experiment_id")
        ).fetchall():
...
def main():
    ...
    run_cmd(
        [
            sys.executable,
            str(root / "data_generator" / "generate.py"),
            "--output-dir",
            str(root / "data"),
            "--seed",
            "42",
