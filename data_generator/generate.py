...
def load_config(path: pathlib.Path) -> Config:
    ...
    seasonal_factors = {int(k): float(v) for k, v in raw.get("seasonal_factors", {}).items()}
...
def generate_members(cfg: Config) -> pd.DataFrame:
    ...
    signup_dates = pd.to_datetime(cfg.start_date) + pd.to_timedelta(signup_offsets, unit="D")
    ...
    df = pd.DataFrame({
        ...
        "age_group": np.random.choice(
            ["18-25", "26-35", "36-45", "46-55", "56+"], size=n,
            p=[0.2, 0.3, 0.25, 0.15, 0.10],
        ),
        "gender": np.random.choice(
            ["female", "male", "other", "prefer_not_to_say"], size=n,
            p=[0.48, 0.48, 0.02, 0.02],
        ),
        "acquisition_channel": np.random.choice(
            ["organic", "social", "paid", "referral"], size=n,
            p=[0.4, 0.3, 0.2, 0.1],
        ),
        "region": np.random.choice(
            ["North America", "Europe", "Asia", "South America", "Other"], size=n,
...
def generate_daily_metrics(
    ...
    df = pd.DataFrame({
        ...
        "resting_heart_rate": np.round(np.random.normal(60, 8, n) * (2 - factors), 2),
        "sleep_hours": np.round(np.clip(np.random.normal(7, 1, n) * (1 + (factors - 1) / 2), 4, 10), 2),
        "sleep_quality": np.round(np.clip(np.random.normal(80, 10, n) * factors, 0, 100), 2),
        "strain": np.round(np.clip(np.random.normal(12, 3, n) * factors, 0, 21), 2),
        "recovery": np.round(np.clip(np.random.normal(70, 15, n) * factors, 0, 100), 2),
...
EVENT_NAMES = [
    "onboarding_completed", "first_sleep_tracked", "first_workout_logged",
    "health_report_viewed", "coaching_tip_opened", "coaching_tip_acted_on",
    "heart_rate_alert_received", "heart_rate_alert_dismissed",
    "health_goal_set", "health_goal_achieved", "share_to_social",
    "invite_sent", "settings_changed", "subscription_upgraded",
    "subscription_downgraded",
]

FEATURE_NAMES = [
    "sleep_tracking", "workout_logging", "coaching", "heart_rate_alert",
    "health_goal", "social", "account_settings", "subscription_management",
]
...
def generate_sessions(
    ...
    starts = pd.to_datetime(signups_rep) + pd.to_timedelta(day_offsets, unit="D") + pd.to_timedelta(sec_offsets, unit="s")
    ...
    locations = np.random.choice(
        ...
        size=total, p=[0.25, 0.20, 0.10, 0.15, 0.15, 0.15],
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=pathlib.Path, default=pathlib.Path("data"),
    ...
    print(f"Generating {cfg.member_count:,} members from {cfg.start_date} to {cfg.end_date}")
